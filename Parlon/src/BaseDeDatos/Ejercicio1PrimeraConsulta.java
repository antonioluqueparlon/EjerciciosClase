...
public class Ejercicio1PrimeraConsulta {
	/**
	 * 
	 */
	private static void pruebaConsultaBasica() {
		try {
			...
			ResultSet rs = s.executeQuery("select * from coche"); // esta es mi consulta
	...
	private static void pruebaConsultaPorFicheroDePropiedades() {
		...
		try {
			...
			// tipo Connection, al cual
			// le tenemos que pasar los parámetros de conexión.
			Connection conexion = (Connection) DriverManager.getConnection("jdbc:mysql://" + host + "/" + schema, user,
					password);
